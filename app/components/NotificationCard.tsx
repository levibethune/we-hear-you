...
function summarizeConditions(conditions: FlowCondition[]): string {
  if (conditions.length === 0) return "All responses";
  const parts = conditions.map((c) => {
    const field = c.field === "source_form_name" ? "form" : c.field === "source_type" ? "source" : c.field;
    const op = c.operator === "equals" ? "is" : c.operator === "not_equals" ? "is not" : c.operator.replace("_", " ");
...
export function NotificationCard({
  ...
  const channel = CHANNEL_LABELS[flow.action_type] || flow.action_type;

  return (
    <div className="soft-card p-4 flex items-center justify-between gap-4">
      <div className="flex-1 min-w-0">
        <Link href={`/dashboard/outputs/notifications/${flow.id}/edit`} className="text-sm font-medium hover:text-accent transition-colors">
          {flow.name}
        </Link>
        <p className="text-xs text-muted mt-0.5 truncate">
          {summarizeConditions(flow.conditions)}
        </p>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <Link
          href={`/dashboard/outputs/notifications/${flow.id}/edit`}
