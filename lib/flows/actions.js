...
function buildPayload(flow, event, response, person, includeFields) {
  const full = {
    ...
    response: {
      ...
      video_embed_url: response.id ? `https://app.wehearyou.io/embed/response/${response.id}` : null,
      created_at: response.created_at,
      // Include all custom analysis fields at the top level for easy mapping in Zapier/Make
      ...Object.fromEntries(
        Object.entries(response.raw_analysis || {})
          .filter(([k]) => !["themes", "mood", "sentiment", "persona", "safety"].includes(k))
          .map(([k, v]) => [`analysis_${k}`, v])
      ),
    },
  };
