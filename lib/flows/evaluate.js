...
export async function evaluateFlows(tenantId, campaignId, event, response, person) {
  ...
  try {
    ...
    for (const flow of flows) {
      ...
      // Evaluate conditions
      if (!matchesConditions(flow.conditions, flow.condition_logic, response, person)) continue;
      ...
      if (safetyReq) {
        ...
        if (safetyReq.no_pii && safety.contains_pii) continue;
        if (safetyReq.no_profanity && safety.contains_profanity) continue;
        if (safetyReq.no_hate_speech && safety.contains_hate_speech) continue;
      }
