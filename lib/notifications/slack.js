  ...
  const payload = {
    ...
    blocks: [
      ...
      ...Object.entries(response.raw_analysis || {})
        .filter(([k, v]) => !["themes", "mood", "sentiment", "persona", "safety"].includes(k) && v != null)
        .map(([k, v]) => ({
          type: "section",
          text: { type: "mrkdwn", text: `*${k.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase())}:* ${Array.isArray(v) ? v.join(", ") : String(v)}` },
        })),
      {
        type: "section",
